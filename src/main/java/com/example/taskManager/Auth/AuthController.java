...
public class AuthController {
    ...
    @PostMapping("/register")
    public ResponseEntity<AuthResponse> registerUser(@RequestBody RegisterRequest request) throws EmailAlreadyRegistered, DefaultRoleNotFoundException {
    ...
    @PutMapping
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<UpdatedUserResponse> updateUserOwnProfile(
        @AuthenticationPrincipal UserDetails userDetails,
        @RequestBody UpdateUserRequestDto requestDto
    ) {
        User user = authService.updateUserOwnProfile(
            userDetails.getUsername(), requestDto.getPassword(),
    ...
    @SecurityRequirement(name = "bearerAuth")
    @PutMapping("/admin/activeState/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> changeActiveState(
         @RequestParam(name = "active") boolean state
    ) {
        authService.changeActiveState(id, state);
        return ResponseEntity.ok("active state of user changed, set - " + state);
    }
    ...
    @GetMapping("/admin/userByRole")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<UserResponseDto>> getUserByRoles(
         @RequestParam String roleName) throws RoleNotFoundException {
        return ResponseEntity.ok(authService.getUserByRoles(roleName));
    }
}
